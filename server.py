...
def find_afterparties():
    ...
    distance_around = distance + measurement
    sort = request.args.get('sort')
    personal_token=os.environ['OATH_TOKEN_PERSONAL']
    payload = {'token':personal_token, 'q':query, 'location.address':location,
    'location.within': distance_around, 'sort_by':sort}
    ...
    if location and distance and measurement:
        ...
        #data['events'][0]['name']['text']

        #data['events'] = list
        #data['events'][0] = dict
        #data ['events'][0]['name'] = dict
        #data['events'][0]['name']['text'] = str

        #look over: for event in data['events'] (a list)
        #append [0]['name']['text'] to events list
        events = []
        for event in data['events']:
            name = data['events'][0][event]['text']
            events.append(name)
